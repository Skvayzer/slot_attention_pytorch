...
dataset_path = 'source/multi_dsprites_colored_on_grayscale.tfrecords'
variant = 'colored_on_grayscale'
ds = dataset(dataset_path, variant)
ds_iter = iter(ds)

for name, l in data_dict.items():
  images = np.zeros((l, 3, 64, 64), dtype=np.uint8)
  masks = np.zeros((l, 6, 1, 64, 64), dtype=np.uint8)
  visibility = np.zeros((l, 6), dtype=float)
  for i in range(l):
    d = dict(next(ds_iter))
